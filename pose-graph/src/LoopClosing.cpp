...
void LoopClosing::setPublishers(ros::NodeHandle &nh)
{
    ...
    for (int i = 1; i < 10; i++)
    	pubPath[i] = nh.advertise<nav_msgs::Path>("path_" + to_string(i), 1000);
}
...
int LoopClosing::detectLoop(KFMatcher* keyframe, int frame_index)
{
    ...
    float min_score = 1.0;
	for (map<KFMatcher*, int>::iterator mit = keyframe->mConnectedKeyFrameWeights.begin(); mit != keyframe->mConnectedKeyFrameWeights.end(); mit++){
		//BowVector neigh_vec;
