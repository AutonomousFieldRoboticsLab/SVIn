...
KFMatcher::KFMatcher(double _time_stamp, vector<Eigen::Vector3d> &_point_ids, int _index, Vector3d &_svin_T_w_i, Matrix3d &_svin_R_w_i, cv::Mat &_image,
...
void KFMatcher::computeBRISKPoint(){
	...
	std::shared_ptr<cv::FeatureDetector> detector(new brisk::ScaleSpaceFeatureDetector<brisk::HarrisScoreCalculator>(
			KFMatcher::briskDetectionThreshold_, KFMatcher::briskDetectionOctaves_,
			KFMatcher::briskDetectionAbsoluteThreshold_,
			KFMatcher::briskDetectionMaximumKeypoints_));

	std::shared_ptr<cv::DescriptorExtractor> extractor(new brisk::BriskDescriptorExtractor(KFMatcher::briskDescriptionRotationInvariance_,
			KFMatcher::briskDescriptionScaleInvariance_));
...
void KFMatcher::updateConnections()
{
	if (KFcounter_.empty())
	{
		std::cout<< "KFcounter is empty for KF: "<<index<< " This SHOULDN't be happening except 1st frame."<<std::endl;
		return;
	}
	...
	for (map<KFMatcher*, int>::iterator mit = KFcounter_.begin(); mit != KFcounter_.end(); mit++){
		if (mit->second > th_weight){
			...
			std::cout<< "Observed Kf: " << mit->first->index <<" with weight(common MapPoint): "<< mit->second<<std::endl;
		}
	}
}
...
int KFMatcher::searchByBoW(KFMatcher* old_kf, vector<cv::Point3f> &vpMatches12, vector<bool> &vbMatched2){
	...
	vpMatches12 = vector<cv::Point3f>(point_3d.size(), cv::Point3f(-1000.0, -1000.0, -1000.0));  // FIXME SHarmin: Better way to initialize
	vbMatched2 = vector<bool>(vMapPoints2.size(),false);
	...
	while(f1it != f1end && f2it != f2end)
	{
		if(f1it->first == f2it->first)
		{
			for(size_t i1=0, iend1=f1it->second.size(); i1<iend1; i1++)
			{
				...
				cv::Point3f pMP1 = point_3d[idx1]; // TODO Sharmin: doulbe check if produces correct result
				const BRIEF::bitset &d1 = window_brief_descriptors[idx1];
				...
				if(bestDist1<TH_LOW)
				{
					//if(static_cast<float>(bestDist1)<0.75*static_cast<float>(bestDist2))  // TODO Sharmin: Decide the ratio: 0.75
					{
						...
						old_pt.x +=  COL;
						cv::line(loop_match_img, point_2d_uv[idx1], old_pt, cv::Scalar(0, 255, 0), 1, 8, 0);
					}
				}
			}
...
Eigen::Vector3d KFMatcher::getLoopRelativeT()
{
...
double KFMatcher::getLoopRelativeYaw()
{
    return loop_info(7);
}
