...
class histogram_equalize:
    def __init__(self, approximate_sync=True):
        ...
        self.left_subscriber = message_filters.Subscriber("/left/image_rect",
                                                          Image,
                                                          queue_size=1000)
        self.right_subscriber = message_filters.Subscriber("/right/image_rect",
                                                           Image,
                                                           queue_size=1000)

        self.left_image_pub = rospy.Publisher("/left_hist/image_rect",
                                              Image,
                                              queue_size=1000)
        self.right_image_pub = rospy.Publisher("/right_hist/image_rect",
                                               Image,
                                               queue_size=1000)
        ...
        self.time_sync = Synchronizer(
            [self.left_subscriber, self.right_subscriber], 100, *slop)
