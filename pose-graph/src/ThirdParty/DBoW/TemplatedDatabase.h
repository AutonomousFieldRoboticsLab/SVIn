...
template <class TDescriptor, class F>
EntryId TemplatedDatabase<TDescriptor, F>::add(const std::vector<TDescriptor>& features,
                                               BowVector* bowvec,
                                               FeatureVector* fvec) {
  BowVector aux;
...
template <class TDescriptor, class F>
void TemplatedDatabase<TDescriptor, F>::query(const std::vector<TDescriptor>& features,
                                              QueryResults& ret,
                                              int max_results,
                                              int max_id) const {
  BowVector vec;
...
void TemplatedDatabase<TDescriptor, F>::query(const BowVector& vec,
  ...
  switch (m_voc->getScoringType()) {
    case L1_NORM:
      // std::cout << "Query Type: L1_NORM"<<std::endl;
      queryL1(vec, ret, max_results, max_id);
      break;

    case L2_NORM:
      std::cout << "Query Type: L2_NORM" << std::endl;
      queryL2(vec, ret, max_results, max_id);
      break;

    case CHI_SQUARE:
      std::cout << "Query Type: CHI_SQUARE" << std::endl;
      queryChiSquare(vec, ret, max_results, max_id);
      break;

    case KL:
      std::cout << "Query Type: KL" << std::endl;
      queryKL(vec, ret, max_results, max_id);
      break;

    case BHATTACHARYYA:
      std::cout << "Query Type: BHATTACHARYYA" << std::endl;
      queryBhattacharyya(vec, ret, max_results, max_id);
      break;

    case DOT_PRODUCT:
      std::cout << "Query Type: DOT_PRODUCT" << std::endl;
...
void TemplatedDatabase<TDescriptor, F>::queryL1(const BowVector& vec,
  ...
  for (vit = vec.begin(); vit != vec.end(); ++vit) {
    ...
    for (rit = row.begin(); rit != row.end(); ++rit) {
      ...
      if ((int)entry_id < max_id || max_id == -1)  // Modified by Sharmin
      {
        // std::cout<<"Entry Id: "<< entry_id <<" MaxId: "<< max_id <<" No enry: "<< m_nentries<<std::endl;
    ...
    for (rit = row.begin(); rit != row.end(); ++rit) {
      ...
      if ((int)entry_id < max_id || max_id == -1) {
        ...
        if (pit != pairs.end() && !(pairs.key_comp()(entry_id, pit->first))) {
          ...
          sums.insert(
              sit, std::map<EntryId, std::pair<double, double> >::value_type(entry_id, std::make_pair(qvalue, dvalue)));
        }
      }
